...
def trep_matr():
	...
	"""if np.abs(a[0]-b[0])>np.abs(a[1]-b[1]):
		 
		p1= ((np.abs(a[0]-b[0]) + np.abs(c[0]-d[0]))/2)
		
		p2= ((np.abs(a[1]-c[1]) + np.abs(b[1]-d[1]))/2)
		print p1,p2
	else:
		
		p1=(np.abs(a[1]-b[1]) + np.abs(c[1]-d[1]))/2
		
		p2=(np.abs(a[0]-c[0]) + np.abs(b[0]-d[0]))/2		
		print 0,180"""
...
#Rotate
def rotate(c):
	if c[0] < 320.0 and c[1] < 240.0:		#the values are dependant upon the pixels of the frame
		rot= 1;
		print 2
		return cv2.getRotationMatrix2D((640/2,480/2),90,1);
		
	elif c[0] < 320.0 and c[1] >= 240.0:
		rot= 2;
		print 2
		return cv2.getRotationMatrix2D((640/2,480/2),0,1);

	elif c[0] >= 320.0 and c[1] < 240.0:
		rot=3;
		print 2
		return cv2.getRotationMatrix2D((640/2,480/2),180,1);

	elif c[0] >= 320.0 and c[1] >= 240.0:
		rot=4;
		print 2
		return cv2.getRotationMatrix2D((640/2,480/2),270,1);
	# cleanup the camera and close any open windows
	camera.release()
	cv2.destroyAllWindows()
	return;
...
def crop (M2,M5):
	...
	espeak.synth("blah blah blah blah to start, say okay ")
	time.sleep(2)
	while True:
		#cmmd=ad.stt()
		if l == 0:
			...
			while True:
				...
				if l==0:
					...
					#frame0 = cv2.warpAffine(frame0,M2,(640,480))					
				...
				#print rows
				#print cols
				#frame = cv2.warpAffine(frame,M2,(640,480))
				...
				if (l==-2 and key == ord('q')) or  (l==-2 and len(pts)>24 and np.abs(dX) < 10 and np.abs(dY) < 10):
					...
					if (np.abs(d-b)>35):
						...
						z=2
					# cleanup the camera and close any openwindows
					camera.release()
					cv2.destroyAllWindows()
					return crop_img;
